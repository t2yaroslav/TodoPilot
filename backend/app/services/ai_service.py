...
- Используй данные о пользователе (психопортрет пользователя) для персонализации
...
async def generate_survey_step(
    ...
    if step == 1:
        ...
        prompt = (
            ...
            "Это должно быть практическое изменение в привычках, организации или подходе, а НЕ задача. "
        ...
        prompt = (
            ...
            "Определи род пользователя по тексту его ответов и профиля и используй соответствующие окончания. "
            "Отвечай на русском языке. "
            "Верни JSON-массив строк. Только JSON, без markdown."
        )
    ...
    messages = [
        {
            "role": "user",
            "content": (
                "Ты — аналитик поведенческих паттернов продуктивности. "
                "Твоя задача — поддерживать актуальный coaching_profile пользователя.\n\n"

                "Правила обновления:\n"
                "1. СОХРАНЯЙ все существующие данные, если новые данные им не противоречат.\n"
                "2. При противоречии — обнови значение, старое перенеси в AI Observations с пометкой [изменение].\n"
                "3. НЕ ВЫДУМЫВАЙ данные — заполняй только то, что подтверждено входными данными.\n"
                "4. Если поле невозможно определить — оставь пустым или сохрани предыдущее значение.\n"
                "5. ОБЯЗАТЕЛЬНО определи пол пользователя по окончаниям глаголов "
                "(сделала → ж, закончил → м). Если пол уже указан — сохрани, обнови только при явных новых данных.\n"
                "6. В секции AI Observations — не удаляй старые записи, добавляй новые с текущей датой. "
                "Фиксируй паттерны, противоречия, изменения в поведении.\n"
                "7. В секции Tried Methods — добавляй только если пользователь явно упоминает опыт использования метода.\n"
                "8. Пиши кратко, в третьем лице, на русском языке.\n\n"

                f"Текущий профиль пользователя:\n{current}\n\n"
                f"Новые данные из еженедельной ретроспективы:\n{survey_text}\n\n"

                "Верни обновлённый профиль строго в следующей структуре:\n\n"
                "# Coaching Profile\n\n"
                "## Identity\n"
                "- Name: \n"
                "- Gender: (м / ж)\n"
                ...
                "- Life stage: \n\n"
                "## Work Style\n"
                "- Chronotype: (жаворонок / сова / нет паттерна)\n"
                "- Peak productivity hours: \n"
                "- Preferred work blocks: (pomodoro 25min / deep work 90min / ...)\n"
                "- Environment: (офис / удалёнка / гибрид)\n"
                "- Planning preference: (детальный план / гибкий фреймворк / по ситуации)\n\n"
                "## Personality & Values\n"
                "- Core values: \n"
                "- Decision style: (аналитик / интуитивный / быстрый)\n"
                "- Risk tolerance: (низкая / средняя / высокая)\n"
                "- Reaction to pressure: (мобилизуется / замирает / избегает)\n\n"
                "## Patterns & Tendencies\n"
                ...
                "- Reaction to failure: \n"
                "- Self-talk tendency: (критик / нейтральный / поддерживающий)\n"
                "- Perfectionism level: (низкий / средний / высокий)\n\n"
                "## Communication Preferences\n"
                "- Coaching tone: (мягкий / директивный / сократический / ...)\n"
                "- Motivation style: (внутренняя / внешняя / accountability)\n"
                "- Feedback preference: (прямой / через вопросы / sandwich)\n"
                "- Language: \n\n"
                "## Tried Methods\n"
                "| Method | Period | Result | Notes |\n"
                "|--------|--------|--------|-------|\n"
                "| | | | |\n\n"
                "## AI Observations\n"
                "- [date] ...\n\n"

                "ТОЛЬКО markdown-документ в указанной структуре. "
                "Без вступлений, пояснений и комментариев вне документа."
            ),
        }
    ]
